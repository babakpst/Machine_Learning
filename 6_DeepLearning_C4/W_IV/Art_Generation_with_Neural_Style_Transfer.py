...
def compute_content_cost(content_output, generated_output):
    ...
    a_C_unrolled = tf.reshape(a_C,[_,-1,n_C])
    a_G_unrolled = tf.reshape(a_G,[_,-1,n_C])
    
    # compute the cost with tensorflow (≈1 line)
    J_content = 1.0/( 4*n_W*n_C*n_H)* tf.reduce_sum( tf.square( tf.subtract(a_C,a_G)))
...
def compute_layer_style_cost(a_S, a_G):
    ...
    #print(a_S.get_shape().get_list())
    # Computing gram_matrices for both images S and G (≈2 lines)
    GS = gram_matrix(a_S)
    GG = gram_matrix(a_G)

    # Computing the loss (≈1 line)
    J_style_layer = 1.0/(4*(n_C*n_W*n_H)**2)*tf.reduce_sum( tf.square( tf.subtract(GS,GG)) )
    print("bbk: ",J_style_layer)
    ### END CODE HERE
