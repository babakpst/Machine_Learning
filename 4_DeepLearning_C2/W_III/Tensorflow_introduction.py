...
def compute_cost(logits, labels):
    ...
    # YOUR CODE STARTS HERE
    #print(logits)
    #soft = tf.nn.softmax(logits, axis = 0)
    #print("softmax \n",soft)
    #print("sum 0: ",sum(soft[:,0]))
    #print("sum 1: ",sum(soft[:,1]))
    #print(labels)
    #print((tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=labels)))
    #cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=labels))
    #print("cross1 \n",     tf.keras.losses.categorical_crossentropy(labels,tf.nn.softmax(logits, axis = 0)))
    #print("cross2 \n",     tf.keras.losses.categorical_crossentropy(labels,logits))
    #print("cross2 \n",     tf.keras.losses.categorical_crossentropy(labels,logits, axis = 1))
    #cost = tf.reduce_mean(tf.keras.losses.categorical_crossentropy(labels,logits, from_logits=True))
    #cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels,logits))
    #print(tf.nn.softmax_cross_entropy_with_logits(labels,logits))
    #cost = tf.reduce_mean(tf.keras.losses.categorical_crossentropy(labels,tf.nn.softmax(logits, axis = 0)))
