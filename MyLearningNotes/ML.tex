...
\begin{itemize}
  ...
  \item Manage missing data (drop items with missin target, and impute missing items).
  \item Manage categorical/object feetures (cordinal, one-hot, etc.)
\end{itemize}
...
\section{Time series}
The basic object of forecasting is the time series, which is a set of observations recorded over time. In forecasting applications, the observations are typically recorded with a regular frequency, like daily or monthly. Linear regression is a common method for forecasting. There are two kinds of features unique to time series: time-step features and lag features.
...
To make a lag feature, we shift the observations of the target series so that they appear to have occurred later in time. Here we've created a 1-step lag feature, though shifting by multiple steps is possible too. ag features let you model serial dependence. A time series has serial dependence when an observation can be predicted from previous observations. Adapting machine learning algorithms to time series problems is largely about feature engineering with the time index and lags.

\subsection{Trend}
This portion of the time series can be captured by moving average rolling windows.
