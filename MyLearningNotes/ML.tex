...
\section{SL: Data analysis pipeline}

In the real world, data is rarely perfect. Even the most powerful model architecture can fail if it's fed a messy, inefficient, or unreliable data stream. This is where a robust \textbf{data pipeline} becomes essential. To overcome these hurdles, you will use PyTorch's core data management tools: The \textbf{Dataset} and \textbf{DataLoader} classes.

The \textbf{Dataset} needs to answer three questions:
\begin{itemize}
\item \_\_init\_\_: Initializes the dataset object.
\begin{itemize}
  \item Typically accepts the data path and any transforms to apply.
  \item Optionally loads metadata or labels associated with the data.
\end{itemize}
\item \_\_len\_\_: Returns the number of samples in the dataset.
\item \_\_getitem\_\_: Retrieves a single sample given an index. Loads the image and its label, applies transforms, and returns them.
\end{itemize}
...
Data ingestion/collection $\rightarrow$
Data preparation (cleaning and preprocessing) $\rightarrow$
Pre-processing $\rightarrow$
ML model building $\rightarrow$
Training $\rightarrow$
Evaluation $\rightarrow$
Deployment

\subsection{Data ingestion/collection}
This is raw data collection. The questions we need to ask is:
\begin{itemize}
  \item enough data?
  \item too much data?
  \item representative data? coverage and bias
\end{itemize}

\subsection{Data preparation}
Access, quality, efficiency. We do cleaning, transforming, and organizing. This fixes errors, handling missing values, engineering new features.

\subsubsection{Data cleaning}
\begin{itemize}
\item missing features
\item extraneous features
\item dimension mis-match
\item adequate labeling
\end{itemize}

\subsubsection{Pre-processing}
\begin{itemize}
  \item numerical encoding
  \item normalization
  \item alternate representation
  \item class weights (if we are dealing with a classification, and the there is an imbalance between number of samples for different classes, we need to smooth this out, by defining class weights to increase the penalty for samples from the class with lower number of sampels.).
  \item test/train
\end{itemize}

\subsection{ML model building}
Choosing the right architecture.
\begin{itemize}
  \item How many neurons should you use?
  \item How are they connected?
  \item What kind of layers does the model need?
\end{itemize}
...
\begin{itemize}
  \item How to measure prediction errors?
  \item How to guide the model to improve?
  \item How fast it learns?
  \item
\end{itemize}
...
\begin{itemize}
  \item de-normalization/de-coding
  \item confidence intervals
  \item data presentation (visualization, computational next steps)
\end{itemize}

\subsubsection{Analysis}
\begin{itemize}
  \item validation
  \item hyper-parameters tuning/model selection
  \item statistical summarization
  \item mathematical modeling
\end{itemize}

\subsection{Deployment}
serving, monitoring. At the end we have the prediction.
...
Each neuron in the layer computes $\overrightarrow{w} . \overrightarrow{x}+b$ and passes the value to the next layer. The purpose of the biad $b$ is to allow the sum to be non-zero, if all the inputs are zero (some consider that as another weight, coming into the neuron). Each neuron will learn a feature (such as affordability, quality, vertical line, etc.) by itself.
...
Model parameters: The part of a model that will be tuned by training, weights and bias.

Hyperparameters: tunable aspects of a model not adapted by training: step sizes, iteration, .... .

\subsection{Custom architecture}
Neural network models can be linear (such as MLP), which data just flows to output. But we can have custom architectures as follows.

\begin{figure}
	\centering
	\begin{subfigure}[htbp] %{.5\textwidth}
		\centering
		\includegraphics[scale=0.08]{pics/linearArch.png}
		%\caption{}
		%\label{fig:sub1}
	\end{subfigure}%
	\begin{subfigure}[htbp] % {.5\textwidth}
		\centering
		\includegraphics[scale=0.08]{pics/CustomArch.png}
		%\caption{}
		%\label{fig:sub2}
	\end{subfigure}
	\caption{NN architecture: (a) linear, (b) custom}
	\label{fig:customArch}
\end{figure}


To build non-linear models you need to control over the flow of data through your layers.

\textbf{\textit{Multi-in Multi-out}}: If the input for the model is of different types, for example the picture and the metadata. The output can also be more than one type. 
\textbf{\textit{Parameter sharing}}: When you want to compare if two items/images are the for the same thing, you train one model (parameter sharing), but you pass each item to the model separately. Then, both features/images map to the same feature space.

\subsection{Logistic regression algo with Neural Network}
We use $a$ instead of $f$, which indicate the activation function. Each neuron in a layer does this calculations. To distinguish activations/parameters for each layer, we can use superscrip []. For example, $a_2^{[1]}$ denotes the second neuron in the first layer or the same for parameters $w_2^{[1]}$. The output of layer one $a^{[1]}$ is the input for the second layer $\overrightarrow{w}^{[2]} . \overrightarrow{a}^{[1]}+b^{[2]}$. Input layer is considered as layer 0. The input layers is considered as layer 0, so $\overrightarrow{x}=\overrightarrow{a}^{[0]}$. There are several option for the activation function g() and each neuron can get a different activation function (see \ref{sec:activation}), for example, for sigmoid function, we have:
...
To calculate the gradient for each layer, we need to multiply the gradients from the previous layers, and since these gradients are small numbers, the multiplication will be even smaller. Thus, the weights for early layers of very deep neural network won't change much. Overall, the early layers, learn very little, or not at all. This is known as vanishing gradient problem. 
...
In this case, the neuron changes to a simple linear regression.
...
\textbf{Softmax function} is a generalization of the logistic function.

This is used when we have a multi class classification problem. If you use sigmoid on each layer, it can produce lots of large values. We define the class based on the largest value. However, training a model with sigmoid is also a challenge, especially if we want to turn the last layer into a one-hot vector. The issue with sigmoid in this case is for the classes that are not valid, the derivative is small (both ends of sigmoid is almost flat, and derivate is close to zero), and make it is difficult to train those nodes.

Softmax applies on the entire layer, rather than individually applying on each neuron. This activation only applied to the output layer for a classification problem (should not be used for hidden layers).

The last layer generates a vector of size m by a linear function which is applied to a softmax function. The softmax function converts the last layer into a probability distribution as described below (which can be better interpreted). After applying softmax, each output will be between 0 and 1 and the outputs will add to 1, so that they can be interpreted as probabilities. The larger inputs will correspond to larger output probabilities. It provides probabilities of the input being in each category.
...
The name softmax is coming from the contrast function hard-max, which converts the last layer, to a vector with only one true / 1 label, and the rests would be zero. But with the softmax, all elements have a value and the sum of all values are one. Note also that if number of softmax classes is two, the softmax reduces to logistic regression.
...
\textbf{Preference Bias} On the other hand, we can’t yet answer the question of the preference bias of a neural network (which, if you recall, describes which hypotheses from
...
\subsection{Full equations for forward and backward}

Neural Network with Two Hidden Layers: Full Forward and Backward Propagation
...
\[
Z^{[2]}(x; W^{[1]}, b^{[1]}, W^{[2]}, b^{[2]}) =
W^{[2]}A^{[1]}(x; W^{[1]}, b^{[1]}) + b^{[2]}
\]

\[
A^{[2]}(x; W^{[1]}, b^{[1]}, W^{[2]}, b^{[2]}) =
g^{[2]}(Z^{[2]}(x; W^{[1]}, b^{[1]}, W^{[2]}, b^{[2]}))
\]
...
\[
\frac{\partial \mathcal{L}(\theta; x,y)}{\partial W^{[3]}}
=
\frac{\partial \mathcal{L}(\theta; x,y)}{\partial A^{[3]}}
\cdot
...
\[
=
\frac{\partial \ell(y, A^{[3]})}{\partial A^{[3]}}
\cdot
g^{[3]'}(Z^{[3]})
\cdot
(A^{[2]})^T
...
\[
\frac{\partial \mathcal{L}(\theta; x,y)}{\partial b^{[3]}}
=
\frac{\partial \ell(y, A^{[3]})}{\partial A^{[3]}}
\cdot
g^{[3]'}(Z^{[3]})
...
\[
\frac{\partial \mathcal{L}(\theta; x,y)}{\partial W^{[2]}}
=
...
\[
\frac{\partial \mathcal{L}(\theta; x,y)}{\partial b^{[2]}}
=
...
\[
\frac{\partial \mathcal{L}(\theta; x,y)}{\partial W^{[1]}}
=
\frac{\partial \mathcal{L}}{\partial A^{[3]}}
\cdot
\frac{\partial A^{[3]}}{\partial Z^{[3]}}
...
\[
\frac{\partial \mathcal{L}(\theta; x,y)}{\partial b^{[1]}}
=
\frac{\partial \ell}{\partial A^{[3]}}
\cdot
g^{[3]'}(Z^{[3]})
...
\subsubsection{Residual Networks (ResNets) — Explanation Notes}


\begin{figure}[htbp]
	\begin{center}
		\includegraphics[trim =0cm 0cm 0cm 2cm, clip, scale=0.2]{pics/resnetModel.png}
		\caption{ResNet: Residual Network model}
		%\label{}
	\end{center}
\end{figure}
...
Deep neural networks suffer from optimization difficulties when they become very deep. 
In particular:

\begin{itemize}
	\item Training error may increase with depth (degradation problem)
	\item Gradients may vanish or become unstable
	\item Adding layers does not guarantee better performance
\end{itemize}

Residual networks address these issues by reformulating the learning problem.

% ------------------------------------------------

\textbf{2. Core Idea of Residual Learning}
...
a residual block learns a residual function:
\[
F(x) = H(x) - x
\]
...
Thus, the network does not learn the full transformation, but only the deviation from the input.

% ------------------------------------------------

\textbf{3. Residual Block Definition}
...
\begin{itemize}
	\item $x$ is the input
	\item $F(x, W)$ is a small neural network (e.g., 2--3 layers)
	\item $y$ is the output of the block
	\item the term $x$ is the skip (identity) connection
\end{itemize}

Example structure:
\[
F(x, W) = g_2(W_2 \, g_1(W_1 x))
\]

So the full block becomes:
\[
y = g_2(W_2 \, g_1(W_1 x)) + x
\]

% ------------------------------------------------

\textbf{4. Why Residual Networks Work}

\textbf{\textit{(1) Easier optimization}}

If the optimal mapping is identity:
\[
H(x) = x
\]

then the residual solution becomes:
\[
F(x) = 0
\]

This is much easier for gradient-based optimization to learn than forcing multiple nonlinear layers to approximate identity.

% ------------------------------------------------

\textbf{\textit{(2) Improved gradient flow}}
...
The identity term $I$ ensures that gradients always have a direct path backward.

% ------------------------------------------------

\textbf{\textit{(3) Enables very deep networks}}

Because gradients can flow through skip connections, very deep architectures (50, 101, 152+ layers) become trainable.

% ------------------------------------------------

\textbf{\textit{(4) Feature reuse}}

Skip connections allow:
\begin{itemize}
	\item low-level features to propagate forward
	\item higher layers to refine features instead of relearning them
\end{itemize}
...
A standard deep network builds representations layer by layer:
\[
x \rightarrow H_1(x) \rightarrow H_2(H_1(x)) \rightarrow \cdots
\]

A residual network instead accumulates corrections:
\[
x \rightarrow x + F_1(x) \rightarrow x + F_1(x) + F_2(\cdot) \rightarrow \cdots
\]

Thus, each layer learns how to \textbf{improve} the representation, not replace it. Indeed, it tried to learn update the Original input to get to the right output (Original input + Correction).

% ------------------------------------------------

\textbf{6. Key Insight}

Residual networks reformulate deep learning as:

\[
\text{Learn } F(x) \text{ instead of } H(x)
\quad \text{where } H(x) = F(x) + x
\]
...
\begin{itemize}
	...
	\item scalability to very deep networks
\end{itemize}
...
CNN is still a NN, but with filters, that does the pattern recognition. Each filter can detect one object. 

What does differentiate CNN from a fully connected NN? An image has a lot of pixels. For example, a 224x224 RGB image has $ 224 \times 224 \times 3 = 150528$ pixels. If you want to use this number of pixels as input to just one hidden layer with 64 layers, that is $64 \times 150528 = 9633792$ weights. This is very expensive and most likely results in overfitting. CNN, on the other hand, instead of learning one weight for every pixel, using a filter (ex 3x3) to learn tiny patch across three channels. That is only $3\times3 \times 3=27$ weights and one bias. When we apply a pool, it shrinks the size of the image, which, in practice, allows to influence a larger portion of the image to the filter. The early layers, then, finds the edges and textures, and deeper layers respond to larger patterns, curves, or combinations of shapes (receptive field).

Think of a CNN as a system that:

\begin{itemize}
  \item looks at small local regions of the input (like tiny patches of an image),
  \item learns filters that detect simple patterns (edges, corners),
  \item then combines them to recognize complex structures (shapes, objects).
\end{itemize}
Instead of looking at everything at once, it learns what matters locally.

Traditional neural networks:
\begin{itemize}
  \item treat all inputs equally
  \item have huge numbers of parameters for images
\end{itemize}

CNNs:
\begin{itemize}
  \item exploit spatial structure
  \item reuse the same filter across the input
  \item are much more efficient and scalable
\end{itemize}

Core building blocks of a CNN:
\begin{itemize}
  \item Convolution layer, applies a filter/kernel that slides over the input. Image/input is HxWxC, and the filter is kxkxC.
  \item Activation function, to add non linearity (for example ReLU).
  \item Pooling layer: reduced the spacial sizes while keeping important information. Since we have multiple filters, the total number of numbers increases, pooling allows to reduce the size. What it does it reduced the size of the image by either just using the max number in the region or average of all numbers.
  \item Fully connected layer: the last layer is a regular fully connected layer, used for classification or regression.
\end{itemize}


\textbf{Saliency maps} in computer vision are visual maps that highlight which parts of an image are most important for a model’s prediction.

In simple terms, they show:
\begin{itemize}
	\item where the model is “looking”
	\item which pixels or regions most influenced the output
	\item what the network considers relevant for a class or decision
\end{itemize}

For example, if a model predicts “cat,” a saliency map may highlight the cat’s face, ears, or outline while downplaying the background.

They are commonly used to:

\begin{itemize}
	\item interpret model behavior
	\item debug whether a model is focusing on the right object
	\item explain predictions to humans
\end{itemize}

In CNNs, saliency maps are often computed using the gradient of the output with respect to the input image. Higher gradient magnitude means that small changes in that pixel would affect the prediction more.


Grad-CAM stands for Gradient-weighted Class Activation Mapping. It is a technique to explain CNN predictions by showing which image regions were most important for a chosen class.

\textbf{How it works:}
\begin{enumerate}
	\item Pick a target class score, such as ``dog.''
	\item Compute gradients of that class score with respect to the last convolutional feature maps.
	\item Average those gradients over spatial locations to get one weight per feature map.
	\item Take a weighted sum of the feature maps and apply ReLU.
	\item Upsample the result to image size and overlay it as a heatmap.
\end{enumerate}

\textbf{Intuition:}
\begin{itemize}
	\item Feature maps tell where visual patterns appear.
	\item Gradients tell which patterns increase the chosen class score.
	\item Their weighted combination gives a class-specific importance map.
\end{itemize}

\textbf{Why use Grad-CAM:}
\begin{enumerate}
	\item Model interpretability: see what the model relied on.
	\item Debugging: detect shortcut learning or background bias.
	\item Trust and communication: provide visual evidence for predictions.
\end{enumerate}

\textbf{Strengths:}
\begin{enumerate}
	\item Works with many CNN architectures.
	\item Class-specific, unlike plain activation maps.
	\item No need to retrain the model.
\end{enumerate}

\textbf{Limitations:}
\begin{enumerate}
	\item Coarse localization because deep feature maps are low resolution.
	\item Can be unstable for some cases.
	\item Mainly designed for convolutional layers.
\end{enumerate}

\textbf{Quick contrast:}
\begin{itemize}
	\item Saliency map: pixel-level gradient importance at input.
	\item Activation map: raw response of a filter.
	\item Grad-CAM: class-specific heatmap from gradients plus activation maps.
\end{itemize}



An \textbf{activation map} is a feature map produced by a convolutional layer in a CNN.

It shows where a particular filter is responding strongly in the input image. In other words, it highlights the spatial locations that activate that filter.

Key idea:
\begin{itemize}
	\item Input image goes through a convolutional filter
	\item The filter detects some pattern, like edges, textures, or object parts
	\item The output is an activation map showing response strength across the image
\end{itemize}

What it means:

\begin{itemize}
	\item Bright or high values = the filter strongly detected its pattern there
	\item Low values = little or no match
\end{itemize}

Example:
\begin{itemize}
	\item One filter may respond to vertical edges
	\item Another may respond to fur-like textures
	\item Another may respond to wheels or eyes in deeper layers
\end{itemize}

Difference from saliency maps:
\begin{itemize}
	\item Activation map: comes from internal CNN feature responses
	\item Saliency map: shows which input pixels most affect the model’s prediction
\end{itemize}


\subsection{Diffusion models}
A diffusion model is a generative model that learns to create data (like images, audio, or video) by reversing a gradual noising process.

In simple terms:

\begin{enumerate}
	\item \textbf{Forward process} \\
	Start with a real sample and add a little random noise at each step until it becomes pure noise.
	
	\item \textbf{Reverse process} \\
	Train a neural network to undo that noise step by step.
	
	\item \textbf{Generation} \\
	At inference time, start from random noise and repeatedly denoise. After many steps, you get a new realistic sample.
\end{enumerate}

\textbf{Why it works:}
\begin{itemize}
	\item The model learns the structure of the data distribution through denoising.
	\item Each step is easier to learn than generating everything in one shot.
\end{itemize}

\textbf{Where you see it:}
\begin{itemize}
	\item Text-to-image systems (for example, Stable Diffusion).
	\item Image editing, inpainting, super-resolution, audio generation, and video generation.
\end{itemize}

\textbf{Trade-off:} \\
High quality and diversity, but often slower sampling than some other generative models because of many denoising steps.
...
\begin{itemize}
	...
	\begin{itemize}
		\item learning rate + scheduler (how to change the learning rate during training)
		\item optimizer
		\item batch size (hardware capacity may dictate the choice here). Small batch means longer to train, but less memory, and helps to avoid local memory. Larger batch sizes, quicker training, more memory, and stuck in local minima.
	\end{itemize}
	\item Regularization to avoid overfitting. Refer to regularization section. 
	\begin{itemize}
		\item weight decay
		\item dropout
		\item Early stopping
...
\subsection{Model selection strategy: Efficiency vs Performance metrics}

Calculating the accuracy of a model (performance metrics such as F1) is not the only reason to choose a model. Efficiency matters too. 


\textbf{Efficiency metrics} include but are not limited to:
\begin{itemize}
	\item \textbf{Model size:} The memory footprint of the model, often measured in megabytes.
	\item \textbf{Inference time:} The average time required for the model to make predictions on new data.
\end{itemize}

To come up with one number for each model, we can normalize performance, inference time, and memory, and add them by a coefficient. 
...
In the attention model, each word decides which other words are more important in the context. For more important words in the sentence, they get more attention (larger attention number will be assigned to them). There different attention heads that can focus on different aspect of a sentence. In pytorch, we can train multiHeadAttentions for LLM. Each attention head is like a filter in CNN, which learns something about the input, in this case a sequence of words. Each word, in the attention model, sees all other words, and to keep track of the order of the words/tokens in the actual sentence, attention models also use positions for each word. Positional info gives a sense of order to the sentence. Attention head is nothing but a NN, that converts the embedded word to another embedding. 
...
Text data is challenging, because:
\begin{itemize}
	\item sequence dependencies and context.
	\item variable length and structure.
	\item vocabulary and representation challenges (model may have not seen all the work, for example in medicine).
	\item ambiguity and polysemy.
\end{itemize}
...
Comments: Text review is different than Image. Image is represented by tensors, but text has no intrinsic meaning. Also, text is sequential and contextual, this is known as polysemy, which means the text meaning depends on context. 

There are different tokenizers: character tokenization based on character (ASCII). Word tokenizers assign one number to each word. One main challenge with the word tokenizers is the vocabularly size. Word normalization removes punctuations (!), contractions, misspeling, and lower case. 


Sentences change in length, but neural networks require fixed length. To overcome the issue, we can use padding method for short sentences or truncation methods for long ones. Padding consequence is extra memory to store, and extra computation for sentences. Truncation the long sentences may cause losing important information. 

The wiser approach is bucketing (grouping sequence of similar length sentences), or smart truncation (prserve the important parts of al ong text).

Next topic is how to represent tokenized senetences in tensor. There many options: a) one list of tokens for each sentence, and a tensor holds multiple sentences. b) one-hot: one tensor of size num of words x num of words in vocab dictionary, for each sentence. All elements of the row is 0, except for that word. This method requires a lot of space. c) bog of words: one row of size "num of words on vocab dict" for each sentence, counting number of repetitions of the word. This method loses the word order information. d) term frequency-inverse document frequency (see the detail in the web). e) The more modern approach is embedding method.
...
The embedded vectors can be added or subtracted, to extract features (king - man keeps the royalty dimensions). Static embedding (just one vector for each word) cannot capture polysemy (example of static embedding: GloVe [Global Vector for Word representation]), instead contextual embeddings allows different embedding, depending on the context, for each word (example: BERT). Despite the benefit, contextual embedding requires more computation.

In short, static embedding  is ideal for simpler tasks, where speed and memory efficiency are prioritized. Contextual embedding is ideal for advanced taskes where subtle differences are critical. 

To visualize the embedded space (with very high dimension), we can use PCA or t-SNE (t-distributed stochastic enighbor embedding), some information may get lost. 

Cosine similarity: the dot product of two embedded vectors shows the similarity of two implementation (close to one similar, 0 orthogonal, -1 opposite).
...
\subsection*{1. Encoder-Only Models (BERT-style)}
\textbf{Goal:} Deeply understand input text.\\
\textbf{Attention pattern:} Bidirectional self-attention (each token attends to tokens on both sides).\\
\textbf{Typical training objective:} Masked language modeling (and sometimes sentence-level auxiliary tasks).\\
\textbf{Best suited for:} Classification, token tagging, retrieval, sentence similarity, ranking/search.\\
\textbf{Strength:} Strong contextual representation learning.\\
\textbf{Limitation:} Not naturally autoregressive for free-form generation.\\
\textbf{Examples:} BERT, RoBERTa, DeBERTa.
...
\subsection*{2. Decoder-Only Models (GPT-style)}
\textbf{Goal:} Generate text token by token.\\
\textbf{Attention pattern:} Causal (masked) self-attention, where each token can only attend to previous tokens.\\
\textbf{Typical training objective:} Next-token prediction.\\
\textbf{Best suited for:} Chat, code generation, open-ended completion, instruction following, summarization.\\
\textbf{Strength:} Simple and highly scalable training objective; excellent generation quality.\\
\textbf{Limitation:} Can be less parameter-efficient for pure understanding tasks than encoder-only models.\\
\textbf{Examples:} GPT-family, Llama-style models, Mistral-style models.
...
\subsection*{3. Encoder--Decoder Models (Seq2Seq, T5-style)}
\textbf{Goal:} Map an input sequence to an output sequence.\\
\textbf{Attention pattern:}
\begin{itemize}
	\item Encoder: bidirectional self-attention over the source input.
	\item Decoder: causal self-attention over generated tokens.
	\item Cross-attention: decoder attends to encoder outputs.
\end{itemize}
\textbf{Typical training objective:} Teacher-forced sequence-to-sequence generation.\\
\textbf{Best suited for:} Translation, summarization, paraphrasing, and structured text transformation.\\
\textbf{Strength:} Clear separation between source understanding and target generation.\\
\textbf{Limitation:} Heavier inference stack than decoder-only in some deployment settings.\\
\textbf{Examples:} T5, BART, mT5, FLAN-T5.
...
\textbf{Intuition in one line:} Encoder-only reads, decoder-only writes, encoder--decoder reads then writes.
...
To do the massive computation for training, we need a lot of memory (RAM), which might not be available on GPU. There are a few other options:
\begin{itemize}
  \item Pruning: remove connections that do not improve the model.
  \item Knowledge distillation: train a smaller model using the original model
  \item Quantization: we can use lower precision data types, to be able to fit the model on the GPU memory. Quantization can be applied to weights and/or activations.
\end{itemize}
...
\textbf{pros}
\begin{itemize}
  \item Reduced memory footprint (more efficient use of GPU memory, enables the training of larger models, enables larger batch sizes)
  \item Increased speed
\end{itemize}
\textbf{Cons:}
Downcasting, when a higher data type, converted to a lower data type, such as an integer, results in loss of data/accuracy.
...
We can use the linear quantization. We need to find scale, and zero point ($r$: is the original unquantized value, and $q$ is the quantized value):
\begin{align}
...
\section{Instruction fine-tuning/Full fine-tuning}
How to fine-tune an LLM with instruction prompts? In the previous section, we learned how to use the few-shot method to improve the learning/output, but this method may not work for smaller models. More importantly, examples take up space in the context window. Instead, we can use fine-tuning to further tune a base model. Instead of few shots, we can try fine-tuning the model. Fine-tuning is a supervised learning process, where we use labeled examples (prompt completion pairs) to update the model. This method is also known as full fine tuning, because all parameters are retuned. The fine-tuned labeled data is divided into training, validation, and test as usual, can be used to train the LLM. At the end, the new model, called the instruct model, is better is responding.

\begin{figure}[H]
  \centering
  \caption{Distributed Data Parallel}
  \includegraphics[trim=0cm 6cm 5cm 0cm,clip,scale=0.15]{pics/finetuning.png}
  %{\footnotesize\textbf{Notes:} XXX.} % changed <<<<<<<<<<
  %#\label{XXX}
\end{figure}

We can fine-tune a model for a single task, but there is a potential downside. Fine-tuning can significantly increase the performance of a model on a specific task, but can lead to reduction in ability on other tasks. This is called \textit{catastrophic forgetting}.

How to avoid catastrophic forgetting?
\begin{itemize}
  \item First, we need to know if this affects your use case. If not, we don't need to worry about it.
  \item Fine-tune on multiple tasks at the same time.
  \item Consider Parameter Efficient Fine-tuning (PEFT).
\end{itemize}

Instruction fine-tuning with FLAN (Fine-tuning LAnguage Net): FLAN models refer to a specific set of instructions used to perform instruction fine-tuning. For example, FLAN-T5 is the FLAN version of T5 model.



From Pytorch course: There are pretrained models in pytorch, that can be improved for a particular datasets with fine-tuning. The next picture shows the general process for a NN model. The initial layers of NN usually learn basic topics such as edges and shapes, so, in a pretrained model, we can freeze initial layers, but unfreeze the rest for better accuracy. If the number of classes of the pretrained model is different from our use case, the last layer can be replaced or adjusted. 
...
Fine-tuning a language model with millions of parameters is computationally expensive and time-consuming. A more pragmatic and often equally effective approach is partial fine-tuning (also known as parameter-efficient fine-tuning (PEFT)).
...
Terminology:
\begin{itemize}
  \item unigram: one word in a sentence
  \item bigram: two (consecutive) words
  \item n-gram: a group of n-words.
\end{itemize}


Traditionally, we use accuracy (correct predictions/Total predictions) to measure how good/accurate the model is. But, this approach is not working for LLM, because the output is a sentence and not easy to check it correct or not.

There are two methods to measure the accuracy for LLMs:
\begin{itemize}
  \item ROUGE (Recall Oriented Under study for (G)jesting Evaluation): is primarily employed to assess the quality of automatically generated summaries by comparing them to human generated reference summaries.
  \begin{itemize}
    \item used for text summarization
    \item compares a summary to one or more reference summaries
  \end{itemize}
  \item BLEU (BiLingual Evaluation Understudy)
  \begin{itemize}
    \item used for text translation
    \item compares to human-generated translations
  \end{itemize}
\end{itemize}

For an answer that is generate by human (reference) and the generated output, you can calculate the metrics as follows:

\begin{align}
  ROUGE-1 Recall = \frac{unigram matches}{unigrams in reference} \\
  ROUGE-1 Recall = \frac{unigram matches}{unigrams in reference} \\
  ROUGE-1 F1 = 2 \frac{precision x recall}{precision + recall}
\end{align}

This metric only focuses on individual words (that is why there is a -1 in the name, which means only one word or unigrams), and it does not consider the ordering of the words. We can define the same metric with pairs of words, bigram, and define the similar metrics. The other metric is ROUGE-L which is based on the Longest Common Subsequence (LCS), which is the number of similar consecutive words in reference and generated output. We can also use clipping to avoid higher score for repeated words.

\begin{figure}
  \centering
  \begin{subfigure}[htbp] %{.5\textwidth}
    \centering
    \includegraphics[trim=3cm 7cm 4cm 0cm,clip,scale=0.1]{pics/rouge1.png}
    %\caption{}
    %\label{fig:sub1}
  \end{subfigure}%
  \begin{subfigure}[htbp] % {.5\textwidth}
    \centering
    \includegraphics[trim=3cm 7cm 4cm 0cm,clip,scale=0.1]{pics/rouge2.png}
    %\caption{one-shot inference}
    %\label{fig:sub2}
  \end{subfigure}
  \begin{subfigure}[htbp] % {.5\textwidth}
  \centering
  \includegraphics[trim=3cm 7cm 4cm 0cm,clip,scale=0.1]{pics/rougel.png}
  %\caption{one-shot inference}
  %\label{fig:sub2}
  \end{subfigure}
  \begin{subfigure}[htbp] % {.5\textwidth}
  \centering
  \includegraphics[trim=3cm 7cm 4cm 0cm,clip,scale=0.1]{pics/rougec.png}
  %\caption{one-shot inference}
  %\label{fig:sub2}
  \end{subfigure}

  \caption{ROUGE metric}
  %\label{fig:test}
\end{figure}

BLEU score is the average of precision across range of n-gram sizes. We need to use libraries to calculate that.

To measure and compare LLMs more holistically, you can make use of pre-exisiting datasets, and associated benchmarks that have been established by LLM researchers specifically for this purpose. There are a couple of these datasets/benchmarks: GLUE, SuperGLUE, MMLU (Massive Multitask Language Understanding), BIG-bench, and HELM (Holistic Evaluation of Language Models).

\section{Parameter Efficient Fine Tuning (PEFT)}

In contrast to full fine-tuning where every model weight is updated during supervised learning, Parameter Efficient Fine Tuning methods only update a small subset of parameters. Some PEFT techniques freeze most model weights and only tune a small subset of trainable layers, for example a particular layer of the model. Some other techniques freeze all model weights, and add new parameters/layers and fine tune only new components. The trained weights are usually around 10~15\% of the original weights. PEFT is less prone to catastrophic forgetting problems of full fine-tuning. Full fine-tuning results in a new version of the model for every task you train on. There are several methods for PEFT, each would trade-offs with
\begin{itemize}
  \item Parameter efficiency,
  \item Memory efficiency,
  \item Model performance,
  \item Inference costs,
  \item Training speed.
\end{itemize}

PEFT methods:
\begin{itemize}
  \item Selective: Select a subset of initial LLM parameters to fine-tune. There are methods to identify which parameters you want to update: certain components of the model or specific layers, or even individual parameter types.
  \item Reparameterize model work with the original LLM parameters, but reduce the number of parameters to train by creating new low rank transformations of the original network weights. Example: LoRA.
  \item Additive methods carry out fine-tuning by keeping all of the original LLM weights frozen and introducing new trainable components. There are two approaches here: adapters add new trainable layers to the architecture of the model, typically inside encoder or decoder components after the attention or feed-forward layers. Soft prompt methods, keep the model architecture fixed and froze, and focus on manipulating the input to achieve better performance, by adding trainable parameter to the prompt embedding or keeping the input fixed and retraining the embedding weights.
\end{itemize}

Low Rank Adaptation (LoRA) freezes most of the original weights (which is now a matrix of size MxN), and injects 2 rank decomposition matrices (of rank r), alongside the original weights. The dimension of the new matrices Mxr and rxN. Then we train the weights of the smaller matrices. Once the new weights are trained for a task (and we can have multiple of these new low rank matrices for different tasks), we multiply the two matrices and add them, and replace them as the original weights in the model. The size of new matrices/parameters are small, and the weights for multiple tasks can be loaded in memory simultaneously. How to choose the size of r is still under investigation. Effectiveness of higher rank appears to plateau (larger r did not improve the performance).

\begin{figure}
  \centering
  \begin{subfigure}[htbp] %{.5\textwidth}
    \centering
    \includegraphics[trim=3cm 7cm 5cm 0cm,clip,scale=0.08]{pics/lora.png}
    %\caption{}
    %\label{fig:sub1}
  \end{subfigure}%
  \begin{subfigure}[htbp] % {.5\textwidth}
    \centering
    \includegraphics[trim=3cm 7cm 5cm 0cm,clip,scale=0.08]{pics/lora2.png}
    %\caption{one-shot inference}
    %\label{fig:sub2}
  \end{subfigure}
  \caption{LoRA (a) process, (b) using LoRA for two/multiple tasks}
  %\label{fig:test}
\end{figure}

Prompt tuning with soft prompts: \\
Prompt tuning is not the same as prompt engineering. Prompt engineering requires a lot of manual work to try different prompts, it is limited by the length of the context window, and output, at the end, might not be performing well yet.

With prompt tuning, we add additional trainable tokens to your prompt and leave it to the supervised learning process to determine their optimal values. These set of trainable tokens are called soft prompt, and gets prepended to embedding vectors that represents your input text. The embedding vectors  have the same length as token vectors and typically 20~100 tokens.

\begin{figure}
  \centering
  \begin{subfigure}[htbp] %{.5\textwidth}
    \centering
    \includegraphics[trim=3cm 7cm 5cm 0cm,clip,scale=0.08]{pics/soft.png}
    %\caption{}
    %\label{fig:sub1}
  \end{subfigure}%
  \begin{subfigure}[htbp] % {.5\textwidth}
    \centering
    \includegraphics[trim=3cm 7cm 5cm 0cm,clip,scale=0.08]{pics/soft2.png}
    %\caption{one-shot inference}
    %\label{fig:sub2}
  \end{subfigure}
  \begin{subfigure}[htbp] % {.5\textwidth}
  \centering
  \includegraphics[trim=3cm 7cm 5cm 0cm,clip,scale=0.08]{pics/soft3.png}
  %\caption{one-shot inference}
  %\label{fig:sub2}
  \end{subfigure}
  \begin{subfigure}[htbp] % {.5\textwidth}
  \centering
  \includegraphics[trim=3cm 7cm 5cm 0cm,clip,scale=0.08]{pics/soft4.png}
  %\caption{one-shot inference}
  %\label{fig:sub2}
\end{subfigure}
  \caption{Soft prompts (a) token addition, (b) hard tokens, (c) soft tokens, (d) multiple prompts  }
  %\label{fig:test}
\end{figure}

Tokens that represent natural language are hard (fixed location in the embedded vector space), but soft prompts are not fixed discrete words of natural language. These are virtual tokens that can take on any value within the continuous multidimensional embedding space, and through supervised learning, the model learns the values for these virtual tokens. With prompt tuning, the weights of model frozen and soft prompts token trained/updated over time. Number of parameters to be tuned here is less than the actual model tuning.  We can tune multiple prompts for each task on top of the LLM, and when we change the task, we can just change the prompt.
One potential issue here is interpretability of learned virtual tokens, because soft prompts can take any value within the embedding space. An analysis of the nearest neighbor tokens to the soft prompt location shows that they form tight semantic clusters. In other words, the words closest to the soft prompt tokens have similar meanings. The words identified usually have some meaning related to the task, suggesting that the prompts are learning word like representations.


\section{Reinforcement learning from human feedback (RLHF)}

Large Language models behaving badly, because large language models trained on vast amount of data:

\begin{itemize}
  \item Toxic language
  \item Aggressive responses
  \item Providing dangerous information
\end{itemize}

The output of the LLM should be Helpful, Honest, and Harmless (HHH). This step is basically trying to align the output with human feedback.

\begin{figure}[htbp]
  \begin{center}
    \includegraphics[trim =30cm 8.0cm 20cm 27cm, clip, scale=0.1]{pics/RLHF.png}
...
Precision (P) is defined as the number of true positives ($T_p$) over the number of true positives plus the number of false positives ($F_p$): $P = \frac{T_p}{T_p + F_p} $. This criterion implies the percentage of true positive (of all examples that are positive, what percentage are actually positive). This means if ML detects a class/category, how likely it is correct. This is per-class. Precision is essential when false positives are costly (e.g., spam detection, medical diagnosis).

Recall (R) is defined as the number of true positives ($T_p$) over the number of true positives plus the number of false negatives ($F_n$): $R = \frac{T_p}{T_p + F_n} $. This criterion implies what percentage of actual positives are correctly recognized. This means if ML can correctly identify the class even if the class is very rare. This is per-class. Recall is vital when false negatives are costly (e.g., disease screening, fraud detection).
...
These quantities are also related to the ($F1$) score, which is defined as the harmonic mean of precision and recall: $F1 = 2 \frac{R * P}{ P + R } $. The F1 score can be interpreted as a harmonic mean of the precision and recall, where an F1 score reaches its best value at 1 and worst score at 0. F1 score allows us to select the best model among many models with different recall and precision. F1 combines both recall and precision. F1 score is ideal when both false positives and false negatives matter, as it provides a trade-off between the two.
...
When we use softmax as the activation, we want to use categorical cross entropy.
The cost is (called Sparse Categorical Cross Entropy)-SparseCtegoricalCrossentropy:
\begin{align}
...
Definition: Compute gradient using the entire training dataset, then update parameters once.
Main advantage: Very stable, low-variance gradient direction, which can make optimization behavior smoother and easier to analyze.
...
Definition: Split data into small batches; for each batch, compute gradient and update parameters.
Main advantage: Best practical tradeoff between speed and stability, with efficient GPU utilization and frequent updates that help convergence.
...
Epoch is a single pass through the all training set. With Batch gradient descent, we do one step of the gradient descent in each epoch, but with the mini-batch gradient descent, we do as many as the number of mini batched gradient descent in each epoch.


\textbf{\textit{Gradient Accumulation}}:
Definition: Run forward/backward on multiple small micro-batches, accumulate gradients, then do one optimizer step after 
k micro-batches.
Main advantage: Simulates a larger effective batch size without needing the memory to hold that large batch at once.


Note:
When people say “compute gradients and update parameters,” they usually mean two separate steps:

\textit{Compute gradients:}
This is the backward pass. In PyTorch, that is typically: loss.backward(). This computes 
 $\partial loss/ \partial \theta$ for each parameter $\theta$ and stores it in param.grad.

\textit{Update parameters:}
This is the optimizer step. In PyTorch: optimizer.step(). This uses the stored gradients to actually change the weights. So:
backward() computes gradients, optimizer.step() updates parameters, optimizer.zero\_grad() clears old gradients before the next iteration.
...
loss.backward()
optimizer.step()
With gradient accumulation, you do backward() several times before one optimizer.step().
...
\item StepLR or staircase decay: reduces the LR by a factor (called Gamma) with each number of epochs. Pytorch can manage that. \\
\item ReduceLROnPlateau: It monitors a particular metric, such as validation accuracy and lower the LR by a factor it the metric does not improve for certain number of epochs, called patience period.
\item CosineAnnealingLR: gradually reduces the LR, using a smaller transition.
\item $\alpha = \frac{1}{1+decayRate\times epochNumber} \alpha_0$ \\
\item $\alpha = 0.95^{epochNumbre} \alpha_0 \; \; \text{(exponential decay)}$ \\
\item $\alpha = \frac{k}{\sqrt{epochNum}}  \alpha_0$ \\
\item Manual decay
\end{itemize}
...
We set a probability to drop units in each layer of neural network. For each sample then we use the reduce network. To implement dropout, we can use the inverted dropout method. In each iteration, we randomly drop some units. There would be no dropout for predictions. It prevents co-adaptation by forcing the model learn robust features instead of relying on shortcuts.
...
\textbf{\textit{weight decay}} discourages the network to use very large weights. Large weight is a sign that model memorizing specific patterns in the training data rather than features that generalize. This is a form of L2 regularization. Very similar to lambda weight?

\textbf{\textit{Early stopping}}: halts the training when the accuracy of the validation stops improving. 


\textbf{\textit{Batch normalization}}: Normalizes the activations in each layer, usually to a mean of 0 and standard deviation of one. 
...
The mean and standard deviation should usually be computed only on the training set. Using statistics computed from the test or validation data can introduce data leakage, where information from the evaluation set influences the training process.
...
BatchNorm2d. This layer is a pivotal technique for building modern, high performing deep neural networks.
...
\begin{itemize}
  \item It Acts as a Regularizer: Because the normalization statistics are calculated for each unique mini batch, it introduces a slight amount of noise into the training process. This noise makes it harder for the model to perfectly memorize the training data, encouraging it to learn more general features and thus reducing overfitting.
  \item It Reduces Sensitivity to Initialization: The layer makes your model less dependent on the specific random weights it starts with, leading to more reliable and repeatable training results.
  \item By adding BatchNorm2d to your CNNBlock, you are not just adding another layer; you are fundamentally making your model's training process more stable, efficient, and robust.
\end{itemize}
